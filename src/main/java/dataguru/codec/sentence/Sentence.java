...
package dataguru.codec.sentence;
...
//    protected LinkedList<IProprietaryTag> tags; // Possible proprietary source tags for the message
    ...
    abstract protected boolean preEncodeProcess(SentenceLine sentence)throws SentenceException;

    /**
     * Abstract method that all sentence classes must implement
     * 
     * The method handles assembly and extraction of the 6-bit gps from sentences. The sentence is expected to be in order.
     * 
    ...
    public int parse(SentenceLine line) throws SentenceException{
        if(preEncodeProcess(line)){
           return this.doParse(line);
        }else
            return 1;
    }
    public abstract int doParse(SentenceLine line) throws SentenceException;
    /**
